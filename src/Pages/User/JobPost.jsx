...
function JobPost() {
    ...
    return (
        <div className='w-full lg:mx-8 p-4'>
            <div className='lg:w-[60%] lg:m-4 space-y-4 sticky-bg-white-blur top-5 '>
                <ul className='flex px-3 w-auto text-gray-400 font-bold  lg:text-xl border-b border-t border-gray-400 rounded-md  justify-between h-[3rem]'>
            ...
            <div className='lg:w-[60%] m-2 lg:m-4 space-y-2 lg:space-y-4 '>
                {   
                    queryData ? queryData?.map((job) => (
                        <div key={job._id}>
                            <Link to={`/user/job/${job &&  job._id}`}>  <JobPostCard title={job.title} company={job?.company_name} location={job.location} posted_by={job.posted_by.full_name} role={job.posted_by.role} /> </Link>
                        </div>
                    ))
                    :
                    jobs && jobs.map((job) => (
